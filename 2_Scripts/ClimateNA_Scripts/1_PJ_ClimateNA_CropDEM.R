...
# Filter DEM files to exclude completed states and Kansas
completed_states <- c(
  "DEM_Arizona.tif", "DEM_California.tif", "DEM_Colorado.tif",
  "DEM_Idaho.tif", "DEM_Kansas.tif", "DEM_Montana.tif", "DEM_Nebraska.tif", 
  "DEM_NewMexico.tif", "DEM_NorthDakota.tif", "DEM_Oklahoma.tif", 
  "DEM_Oregon.tif", "DEM_SouthDakota.tif", "DEM_Texas.tif", "DEM_Utah.tif"
)
dem_files <- dem_files[!basename(dem_files) %in% completed_states]
print(paste("Processing files:", length(dem_files)))

################################################################################
#                            PROCESS EACH DEM
################################################################################
# Iterate through remaining DEM files
for (dem_path in dem_files) {
  print(paste("Processing DEM:", dem_path))
  # Load the current DEM
  dem <- rast(dem_path)
  # Skip DEMs with very small extents
  dem_extent <- ext(dem)
  if ((xmax(dem_extent) - xmin(dem_extent)) < 0.01 || (ymax(dem_extent) - ymin(dem_extent)) < 0.01) {
    print("Skipping DEM due to small extent.")
    next
  }
  ...
  grid_size <- 20  # Adjust grid size if necessary
  grids <- tryCatch({
    st_make_grid(st_as_sfc(st_bbox(ext(dem))), n = c(grid_size, grid_size))
  }, error = function(e) {
    print("Error creating grid. Adjusting grid size.")
    st_make_grid(st_as_sfc(st_bbox(ext(dem))), n = c(5, 5))  # Fallback grid
  })
